...
RROC_sidedresss <-map(Growing_Season_2025$Rosemount_PRS_Data, ~.x[.x$Treatment == "Liquid Dairy Sidedress",]) %>%
  keep( ~nrow(.) > 0)

Avg_PRS_Treatment <- bind_rows(RROC_sidedresss)
Avg_PRS_Treatment <- map_df(Avg_PRS_Treatment, as.numeric)


Avg_PRS_Treatment[] <- lapply(Avg_PRS_Treatment, as.numeric)


Avg_PRS_Treatment <- Avg_PRS_Treatment %>%
  map_df(Avg_PRS_Treatment, as.numeric)

  group_by(Burial) %>%
  summarize(Avg_NO3_Uptake = mean())





P101 <- Growing_Season_2025$Rosemount_PRS_Data$RROC_101
P111 <- Growing_Season_2025$Rosemount_PRS_Data$RROC_111
